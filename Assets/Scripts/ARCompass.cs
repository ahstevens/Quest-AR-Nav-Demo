...
public class ARCompass : MonoBehaviour {
    ...
    Material compassLineMaterial;
    float compassRadius = 20;
    float compassHeight = 1.5f;
    float compassElevation = 0.5f;
    ...
    //Vectrosity:
    List<Vector3> compassRingPoints;
    List<Vector3> bigTickPoints;
    List<Vector3> mediumTickPoints;
    List<Vector3> smallTickPoints;

   VectorLine compassRingLine;
   VectorLine compassBigTicksLines;
   VectorLine compassMediumTicksLines;
   VectorLine compassSmallTicksLines;

    List<Vector3> headingLinePoints;
    VectorLine headingLine;
    Texture2D headingLineTexture;
    int headingLineNumSegs;

    Texture2D LimitLineTexture;
    List<Vector3> leftTurnLimitLinePoints;
    VectorLine leftTurnLimitLine;
    int leftTurnLimitLineNumSegs;
    List<Vector3> rightTurnLimitLinePoints;
    VectorLine rightTurnLimitLine;
    ...
    VectorLine viewDirectionToHorizonLine;
    ...
    VectorLine viewDirectionDistanceLine;
    ...
    void Start()
    {
        ...
        compassLineMaterial = Resources.Load("materials/CompassLineMaterial", typeof(Material)) as Material;
        ...
        //testNavLineTexture = Resources.Load("textures/NavLineTextureBold2", typeof(Texture2D)) as Texture2D;

        //for drawing
        compassRingPoints = new List<Vector3>();
        ...
        for (int i = 0; i < 360; i++)
        {
            ...
            compassRingPoints.Add(new Vector3(thisX, 0, thisY));
        ...
        //add first vert to last position because linerenderer.loop is missing???
        compassRingPoints.Add(compassRingPoints[0]);
        ...
        //add heading icon
        //headingIndicatorSprite = (Sprite)AssetDatabase.LoadAssetAtPath("Assets/GUI/HeadingIndicator.png", typeof(Sprite));
    ...
    void Update () {
        ...
        transform.position = new Vector3(vrCameraObject.transform.position.x, compassElevation, vrCameraObject.transform.position.z);
        //transform.SetPostionAndOrientation(new Vector3(vrCameraObject.transform.position.x, compassElevation, vrCameraObject.transform.position.z), Quaternion.identity);
    ...
    void AddFixedTextLabel(string label, double degrees, float fontSize, double height)
    {
